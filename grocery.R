...
text <- text [text$product_category %in% c("GROCERY_SNACK_BEVERAGE"), ]
...
text_new %>% write_csv("Desktop/UCLA/Amazon/data/grocery_text.csv")
...
toks <- text_new$combined_text %>% 
  ...
  tokens_select(c("amazon", "whole", "food", "foods", "everyday", "value"), selection = "remove", case_insensitive = TRUE) %>% 
...
result <- print(sort(cos_dtm[,"text2384"],dec=T)) %>% 
  as.data.frame() %>% 
